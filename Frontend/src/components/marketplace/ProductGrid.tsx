    ...
    <div className="flex-1 font-sans flex overflow-x-hidden" style={{ minHeight: "100vh" }}>
      ...
      <div className={`flex-1 transition-all duration-300 ${selected && !isMobile ? "pr-0 md:pr-[440px]" : ""}`}>
        ...
        <div className="w-full">
          <div className={`grid ${selected && !isMobile ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3"} gap-4 sm:gap-6 lg:gap-8 auto-rows-fr`}>
            {projects.map((project) => (
              <Card
                key={project.id}
                className="group hover:shadow-xl transition-all duration-300 hover:scale-102 bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col h-full"
              >
                <div className="relative">
                  <img
                    src={project.image || NoImageAvailable}
                    alt={project.title}
                    className="w-full h-32 sm:h-40 object-cover group-hover:scale-105 transition-transform duration-300 cursor-pointer"
                    loading="lazy"
                ...
                <CardContent className="p-3 sm:p-4 flex flex-col flex-1">
                  ...
                  <h3 
                    className="text-sm sm:text-base font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors cursor-pointer line-clamp-2"
                    onClick={() => setSelected(project)}
                  >
                    {project.title}
                  </h3>
      ...
      {selected && isMobile && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-end sm:items-center justify-center">
          <div className="bg-white w-full h-[90vh] sm:h-[80vh] sm:w-[95%] sm:max-w-md rounded-t-xl sm:rounded-xl overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">Posted {selected.posted}</span>
              </div>
              <Button
                size="icon"
                variant="ghost"
            ...
            <div className="overflow-y-auto flex-1 px-4 py-4">
              ...
              <Button 
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg shadow transition-all duration-300 mb-4"
                onClick={() => handleViewDetails(selected.id)}
              >
                Let's Connect
              </Button>
