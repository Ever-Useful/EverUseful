      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-base text-slate-600 mb-8 max-w-2xl mx-auto leading-relaxed">
            ...
            {!showAllFreelancers && filtered.length > 8 && (
              <span className="block text-sm text-slate-500 mt-1">
                Showing 8 of {filtered.length} freelancers
              </span>
            )}
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-red-50 text-red-700 rounded-full">
              <span className="text-sm font-medium">{error}</span>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-12">
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-slate-50 text-slate-700 rounded-full">
              <span className="text-sm font-medium">No freelancers found matching your criteria</span>
            </div>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
              {filtered.slice(0, showAllFreelancers ? filtered.length : 8).map((f, idx) => (
            ...
            {filtered.length > 8 && (
              <div className="text-center mt-12">
                <button
                  ...
                  className="group inline-flex items-center gap-2 px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold rounded-full transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl text-sm sm:text-base"
                >
                  {showAllFreelancers ? 'Show Less' : `View All ${filtered.length} Freelancers`}
                  <ArrowRight className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${showAllFreelancers ? 'rotate-180' : 'group-hover:translate-x-1'}`} />
                </button>
              </div>
            )}
          </>
